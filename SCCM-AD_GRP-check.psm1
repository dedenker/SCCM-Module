...
# If this is not the case the office is "not SCCM ready".				#
# For now only works on EMEA region (this will be adjusted with a dropdown box)		#
#											#
#This is just a prove of concept, so things are still a mess				#
#											#
#########################################################################################
...
function groupExists ([string]$name){
	$string = "LDAP://CN=GRP_APP_$name,OU=Applications,OU=Security Groups,OU=GIS,DC=TMF-Group,DC=COM"
	write-debug $string
...
#	Searchbase EMEA - APAC - SCNA or Global!
	Foreach ($location in (Get-ADOrganizationalUnit -Filter 'Name -like "Security Groups"' -SearchBase 'OU=EMEA,DC=TMF-GROUP,DC=COM' | Get-ADObject))
	{
...
if($result -eq 6){
	...
	if(groupExists ($arg))
	{
		...
		New-ADGroup -Path "OU=Applications,OU=Security Groups,OU=GIS,DC=TMF-Group,DC=COM" -Name "GRP_APP_$arg" -GroupScope Global -GroupCategory Security -Description "Machine name based (Created by SCCM Application Script)"
	}
